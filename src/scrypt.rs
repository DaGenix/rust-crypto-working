...
mod test {
    ...
    use super::{scrypt, scrypt_simple, scrypt_check, ScryptParams};
    use testutil::{parse_tests, read_opt_u32, read_data};

    #[test]
    pub fn test_scrypt() {
        let scrypt_tests = parse_tests("testdata/scrypt.toml", "test-scrypt");
        for test_table in scrypt_tests {
            let test = test_table.as_table().expect("Test data must be in Table format.");
            let log_n = read_opt_u32(test, "log_n").expect("log_n is required.");
            assert!(log_n < 256);
            let log_n = log_n as u8;
            let r = read_opt_u32(test, "r").expect("r is required.");
            let p = read_opt_u32(test, "p").expect("p is required.");
            let password = read_data(test, "password");
            let salt = read_data(test, "salt");
            let expected = read_data(test, "expected");

            let mut result: Vec<u8> = repeat(0).take(expected.len()).collect();
